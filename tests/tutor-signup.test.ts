...
test("several curricula are accepted, unknown ones are not", () => {
  const form = new FormData();
  form.append("curriculum", "IB");
  form.append("curriculum", "SAT");
  form.append("curriculum", "AP");
  form.append("curriculum", "IB");
  assert.deepEqual(tutorSignupDetailsFromForm(form).curricula, ["IB", "SAT", "AP"]);

  assert.equal(
    validateTutorSignupDetails({ ...validDetails(), curricula: ["IB", "SAT", "AP"] }),
    null,
  );
  assert.equal(
    validateTutorSignupDetails({ ...validDetails(), curricula: ["IB", "GCSE"] }),
    "curriculum",
  );
});

test("an empty introduction is accepted and an oversized one is not", () => {
  assert.equal(validateTutorSignupDetails({ ...validDetails(), introduction: "" }), null);
  assert.equal(
    validateTutorSignupDetails({ ...validDetails(), introduction: "가".repeat(2001) }),
    "introduction",
  );
});
